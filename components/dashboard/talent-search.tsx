...
import { Users, Search, Loader2, BookOpen, Briefcase, MapPin, Star } from 'lucide-react'
import { apiClient } from '@/lib/api'
import { useRequireAuth } from '@/lib/auth-context'
import Link from 'next/link'

export function TalentSearch({ role, title, description }: { role: string; title: string; description: string }) {
  ...
  const fetchCandidates = async () => {
    try {
      const c = await apiClient('/students/search?limit=10')
      ...
      <div className="space-y-6 max-w-6xl">
        ...
        <Card glass>
          <CardHeader>
            ...
            <CardDescription>Students matching your criteria</CardDescription>
          </CardHeader>
          <CardContent>
            {loading || searching ? (
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center justify-end gap-2 mt-4 pt-4 border-t border-white/5">
                          <Link href={`/messages`} className="inline-flex h-8 items-center justify-center rounded-md text-xs bg-blue-600 px-3 py-1 hover:bg-blue-500 text-white transition-colors">
                            Contact
                          </Link>
                        </div>
                      </div>
